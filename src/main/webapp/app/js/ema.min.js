...
app.config(['$stateProvider', '$urlRouterProvider',
    function ($stateProvider, $urlRouterProvider) {
        ...
        $stateProvider
            ...
            .state('tab.integral-success', {
              url: '/integral-success',
              views: {
                  'tab-me': {
                      templateUrl: 'templates/me/integral-success.html',
                      //controller: 'recordCtrl'
                  }
              }
            })
            .state('tab.integral-fail', {
              url: '/integral-fail',
              views: {
                  'tab-me': {
                      templateUrl: 'templates/me/integral-fail.html',
                      //controller: 'recordCtrl'
                  }
              }
            })
...
app.constant('config', {
    ...
    "pointWithdraw": "/userPointApi/apply-point",
    "getPointRecord": "/userPointApi/get-pointRecord",
    // index
    "getPointRanking": "/pointApi/get-pointRanking2",
    "getPaopaoClick": "/pointApi/gather-energyBall2",
    //我的任务
...
app.controller('invoiceListCtrl', ['$scope', '$state', '$ionicHistory', 'server', 'appModel', 'popup',
    function ($scope, $state, $ionicHistory, server, appModel, popup) {
        ...
        $scope.createInvoiceNew = function () {
            $state.go('tab.invoice-edit', {
            });
        };
...
app.controller('indexPaopaoCtrl', ['$scope', '$timeout', '$state', 'server', 'config', 'appModel',
    function ($scope, $timeout, $state, server, config, appModel) {
        ...
        $scope.nowtime = new Date().getHours()*60 + new Date().getMinutes()
        ...
        $scope.initPage = function () {
            ...
            server.getPointRanking(appModel.get("userInfo").uid).then(function (data) {
                ...
                for (var i = 0; i < $scope.pointBalloonList.length; i++) {
                    var countdown = $scope.pointBalloonList[i].time - $scope.nowtime
                    ...
                    $scope.paopaotime[i] = countdownshow
                }
            });
        }
        // 数据更新
        ...
        $scope.getMessage();
        // 定时刷新
        setInterval($scope.getMessage,60000);
        $scope.paopaoClick = function ($index,item) {

            console.log($scope.pointBalloonList);
            if (item.brandEnergyStatus == 1) {
                console.log('摘取过');
            }else if (item.energyStatus==0&&item.brandEnergyStatus != 1) {
                console.log('未成熟');
                // $scope.getMessage();
            }else {
                // $scope.paoIndex[$index] = 'gray'
                // if (item.type==1) {
                //
                //
                // }else if(item.type==2){
                //     $scope.paoIndex[$index] = 'hide'
                //     // $scope.initPage();
                // }
                ...
                var paoId = {
                    uid: appModel.get("userInfo").uid,
                    pointBalloonId : item.id
                }
                server.getPaopaoClick(paoId).then(function (data) {
                    console.log(data);
                    console.log($scope.pointBalloonList);
                });
                $scope.getMessage();
...
// 我的任务
app.controller('MyTaskListCtrl', ['$scope', 'popup','$timeout', '$state', 'server', 'config', 'appModel',
    function ($scope, popup, $timeout, $state, server, config, appModel) {
    ...
    $scope.giveUpTask = function () {
        ...
        server.getGiveUpTask($scope.taskJid).then(function (data) {
            // // console.log(data);
            if (data ==1) {
                return popup.showTips('任务放弃成功');
                $scope.initPage();
            }else {
                return popup.showTips('任务放弃失败');
...
app.controller('tackDetailsCtrl', ['$scope', '$stateParams', '$state', '$filter', 'server', 'wechat', 'appModel', 'popup', 'config','$ionicHistory',
    function ($scope, $stateParams, $state, $filter, server, wechat, appModel, popup, config,$ionicHistory) {
        ...
        $scope.initPage = function () {
            server.getTaskDetail(psid).then(function (data) {
                $scope.TaskDetail = data;
                if ($scope.TaskDetail.endTime<$scope.date) {
    ...
    function ($scope, $stateParams, $interval, $state, server, popup, config, appModel) {
        ...
        $scope.signIn = function () {
            ...
            if (!phoneReg.test(mobile)) {
                return popup.showTips('手机号码格式不正确！');
            }else if(!/^\d{17}(\d|x)$/i.test(sId)) {
                return popup.showTips('你输入的身份证长度或格式错误');
            }else if(aCity[parseInt(sId.substr(0,2))]==null){
                return popup.showTips('你的身份证地区非法');
            }else if(sBirthday!=(d.getFullYear()+"-"+ (d.getMonth()+1) + "-" + d.getDate())){
                return popup.showTips('身份证上的出生日期非法');
            }
            else {
                var req = {
                    jobId: $scope.jobId,
...
//我的积分
app.controller('pointCtrl', ['$scope', 'server', 'appModel', function ($scope, server, appModel) {
    ...
    $scope.getPointList = function () {
        server.getPointList(reqParam).then(function (data) {
            // console.log(data);
            $scope.pointList = data;
            // console.log(data.userAgent.point);
        });
    };
    $scope.getPointList();
...
//积分提现
app.controller('withdrawCtrl', ['$scope', '$interval', '$state', 'server', 'popup', 'config', 'appModel',
    function ($scope, $interval, $state, server, popup, config, appModel) {
        $scope.initPage = function () {
        };
        $scope.user = {referralcode: ''};
        ...
        $scope.onShow = false;
        $scope.user.payWay = '2';
        $scope.point = appModel.get("userInfo").point;

        $scope.showPayWay = function(){
            // console.log($scope.user.payWay)
        }
        ...
        $scope.sendValidateNum = function () {
            ...
            popup.showConfirm({title:'发送短信',msg: ' 是否向'+phone+'注册手机号码 发送短信？!', success: onSuccess, fail: onFail});
            function onSuccess() {
                if ($scope.isVaild) {
                    $scope.timer = maxTime;
                    $scope.transmit = "已发送";
        ...
        $scope.withdraw = function () {
            var phone = $scope.user.username;
            var point = $scope.user.point;
            var qpwd = $scope.user.qpwd;
            if (!phoneReg.test(phone)) {
                return popup.showTips('手机号码格式不正确！');
            }
            if ($scope.user.point == '') {
                return popup.showTips('请输入提现积分!');
            }else if( $scope.user.point >  $scope.point){
                return popup.showTips('提现积分不能大于可提现积分!');
            }
            if ($scope.user.code != identifyCode) {
                return popup.showTips('验证码错误!');
            }
            ...
            var req = {
                ...
                type: $scope.user.payWay
            };

            flag = false;
            server.pointWithdraw(req).then(function (data) {
                popup.alert({btnText: '确认', msg: '申请提现成功!', success: onSuccess});
                function onSuccess() {
                    flag = true;
                    $state.go('tab.integral-success');
                }
            }, function (error) {
                flag = true;
                popup.alert({btnText: '确认', msg: error.message, success: onSuccess});
                function onSuccess() {
                    flag = true;
                    $state.go('tab.integral-fail');
                }
                //popup.alert({btnText: '确认', msg: error.result.success});
            });
...
//根据月份筛选提现记录
app.controller('recordCtrl', ['$scope', 'server', 'appModel', function ($scope, server, appModel) {
    var popup=document.getElementById('popup');

    $scope.calendarShow = function (obj){
        popup.style.display = 'block'
    }
    $scope.calendarHide = function (obj){
    ...
    var m =$scope.now.getMonth() + 1;
    m = m < 10 ? "0" + m : m;
    // console.log(m);
    var nowYear = $scope.now.getFullYear()
    // console.log(nowYear);

    $scope.theMonth = '本月';
    $scope.monthData = [];
    for(var i = 0; i < 12; i++) {
        $scope.now.setMonth($scope.now.getMonth() - 1);
        var m =$scope.now.getMonth() + 1;
        m = m < 10 ? "0" + m : m;
        //在这里可以自定义输出的日期格式
        $scope.monthData.push($scope.now.getFullYear() + "-" + m);
        //$scope.monthData.push(d.getFullYear() + "年" + m + '月');
    }
    ...
    $scope.recordData = [];
    $scope.getPointRecord = function () {
        popup.style.display = 'none'
        $scope.theMonth = '本月';
        server.getPointRecord(reqParam).then(function (data) {
            // console.log(data);
            $scope.recordData = data;
        });
    };
    $scope.getPointRecord();

    //根据月份筛选提现记录

    $scope.showMonthData = function (item) {
        // console.log(item);
        $scope.theMonth = item;
        popup.style.display = 'none'
        var reqParam = {
...
core.service('server', ['$state', 'api', 'config', 'appModel', '$ionicHistory',
    function ($state, api, config, appModel, $ionicHistory) {
        ...
        this.getPaopaoClick = function (paoId) {
            var url = config.getPaopaoClick + '?uid=' + paoId.uid + '&pointBalloonId=' + paoId.pointBalloonId ;
            return api.call(url, null, 'GET', true);
        };
        // 我的任务
         ...
         * @returns {*}
         */
        this.pointWithdraw = function (param) {
            var uid = param.uid;
            var point = param.point;
            var mobile = param.mobile;
            var type = param.type;
            var url = config.pointWithdraw + '?uid=' + uid + '&point=' + point + '&mobile=' + mobile + '&type=' + type;
            return api.call(url, null, 'GET', true);
        };
         ...
         * @returns {*}
         */
        this.getPointRecord = function (param) {
            var uid = param.uid;
            var rows = param.rows || 1;
            var page = param.page || 20;
            var month = param.month;
            var url = config.getPointRecord + '?uid=' + uid + '&rows=' + rows + '&page=' + page + '&month=' + month;
            return api.call(url, null, 'GET', true);
        };
